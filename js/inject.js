...
function formatBreaks(p) {
	...
	if (halfs[p] > 0) {
		text = text + "50+ (" + halfs[p] + ") ";
	}

	if (centuries[p] > 0) {
		text = text + "100+ (" + centuries[p] + ")";
	}
...
$("#iface-content table tbody tr").each(function() {
	var tag = $(this).find("td:eq(1) a");
	if (!tag.has('img').length) {
		var player = tag.text().trim();
		players[player]  = ~~players[player] + 1;
	}
}).each(function() {
	var a = $(this).find("td:eq(1) a");
	var name = a.text().trim();
	if(name.length && typeof players[name] !== 'undefined') {
		a.append(' (' + players[name] + ')');
	}
})
...
$(window).on('load', function() {
	...
	if (container.length) {
		...
		function getLabels(container) {
			bestof = +container.find("p.frames").text().match(/\d+/g).pop();
		...
		var div_flow = $(document.createElement("canvas"))
			.attr("id", "match-chart")
			.css("backgroundColor", "#e7e7e7");

		var div_probability= $(document.createElement("canvas"))
			.attr("id", "match-probability")
			.css("backgroundColor", "#e7e7e7");

		container.append(div_flow);
		container.append(div_probability);

		var ticks = flow.map((val) => parseInt(val[1]));

		var flow_chart = new Chart(div_flow, {
			type: 'line',
			data: {
		...
		var flow_probability = new Chart(div_probability, {
			type: 'line',
			data: {
				labels: getLabels(container),
				datasets: [{
					data: flow.map(v => getProbability(v)),
					borderColor: '#11119f',
					backgroundColor: '#e7e7e7',
					borderWidth: 2,
					tension: 0,
...
select.on("change", function(event) {
	...
	$("table#sessions tbody tr").each(function() {
		...
		var hours = td.html().trim().substring(0, 5);
		var gmt_shift = +event.target.value;
		...
		td.append(
			"<p>TMZ: " + getTimeShifted(hours.split(":"), gmt_shift) + "</p>"
		);
	});
